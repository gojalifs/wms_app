<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @endif
            <form method="GET" action="{{ route('intake.index') }}" class="mb-4 flex items-center gap-3">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('wms.intake.search_placeholder') }}"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $order->supplier }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $order->received_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $order->user->name }}</td>
                                <td class="px-6 py-4 text-sm text-right">
                                    <a href="{{ route('intake.show', $order) }}"
                                        class="text-indigo-600 hover:text-indigo-900">{{ __('wms.general.view') }}</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-gray-400">{{ __('wms.intake.no_data') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
